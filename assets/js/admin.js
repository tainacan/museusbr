/**
 * Scripts carregados no lado administrativo do site
 */
function museusBrRenameLabels() {
    if (wp && wp.hooks) {
        function tainacanItemEditionItemLoaded (collection, item) {
     
            if ( collection.id == museusbr_theme.museus_collection_id ) {
                tainacan_plugin.i18n.label_ready_to_create_item = 'Solicitação de cadastro';
                tainacan_plugin.i18n.instruction_create_item_select_status = item.status && item.status != 'publish' ? 'Pronto para enviar a solicitação de cadastramento deste museu?' : 'Pronto para atualizar o cadastro deste museu?';
            ...
            tainacan_plugin.i18n.label_create_item = 'Concluir'; 
            ...
            tainacan_plugin.i18n.status_pending = 'Enviado para análise';
            tainacan_plugin.i18n.label_send_to_review = 'Enviar para análise';
        }
        wp.hooks.addAction('tainacan_item_edition_item_loaded', 'tainacan-hooks', tainacanItemEditionItemLoaded);
    }
}
museusBrRenameLabels();

function museusBrRedirectItemEdition() {
    if ( wp && wp.hooks ) {
        function getTainacanItemEditionRedirect (itemEditionRedirect, itemObject, itemId) {
    
            if ( !museusbr_theme.current_user_is_gestor )
                return itemEditionRedirect;

            // Se o gestor está criando ou editando um museu, volta para a página dele
            if ( itemObject.collectionId == museusbr_theme.museus_collection_id ) {
                return museusbr_theme.museus_admin_page_url + '&post=' + itemId;
            }
